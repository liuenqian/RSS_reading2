...
//   node batch_download.mjs --title "<exact title>" [--doi <doi>] [--pmid <id>] [--pmcid <id>] --out <dir> [--open-access]
...
import {
  discoverOpenAccessCandidates,
  filenameForPdfUrl,
  findArxivByTitle,
  normalizeDoi,
} from "./lib/open-access.mjs";
import {
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (k === "--doi") a.doi = argv[++i];
    else if (k === "--pmid") a.pmid = argv[++i];
    else if (k === "--pmcid") a.pmcid = argv[++i];
...
async function downloadDoi(proxy, doi, outDir, wantSi, debug, { cleanBundle = false, bundleTitle = "" } = {}) {
  ...
  try {
    ...
    const basePdfPath = cleanBundle
      ? path.join(bundleDir, `${safeArticleTitle(articleTitle)}.pdf`)
      : path.join(outDir, "PDFs", safe + ".pdf");
    const pdfPath = uniqueFilePath(basePdfPath);
    for (const pdfUrl of meta.cand) {
      const safety = await validatePublicDownloadUrl(pdfUrl);
      if (!safety.ok) {
        if (debug) process.stderr.write(`[security] rejected PDF URL: ${safety.reason}\n`);
        continue;
      }
...
async function downloadPdfUrl(proxy, pdfUrl, outDir, title = "", { allowBrowser = true } = {}) {
  const fileName = filenameForPdfUrl(pdfUrl, title);
  const outPath = uniqueFilePath(path.join(outDir, "PDFs", fileName));
  const direct = await fetchDirectToFile(pdfUrl, outPath).catch((error) => ({
    ok: false,
  ...
  if (direct.ok) {
    return {
      ...
      file: direct.file,
      bytes: direct.bytes,
      via: direct.finalUrl || pdfUrl,
      transport: "direct",
      attempts: direct.attempts || 1,
    };
  }
  if (/^(?:HTTP (?:404|410)\b|unsafe_url:)/.test(direct.err || "")) {
    return {
      title,
      status: STATUS.PDF_FETCH_FAILED,
      url: pdfUrl,
      err: `direct: ${direct.err}`,
      attempts: direct.attempts || 1,
    };
  }
  if (!allowBrowser) {
  ...
  try {
    ...
    const got = await fetchToFile(proxy, tab, pdfUrl, outPath);
    if (got.ok) {
      return {
        title,
        status: STATUS.DOWNLOADED,
    ...
    return {
      ...
      status: STATUS.PDF_FETCH_FAILED,
      url: pdfUrl,
      err: `direct: ${direct.err}; browser: ${got.err}`,
      attempts: (direct.attempts || 1) + 1,
    };
  } finally {
    await closeTab(proxy, tab);
...
async function downloadResolvedItem(args) {
  const discovery = await discoverOpenAccessCandidates({
    doi: args.doi,
    pmid: args.pmid,
    pmcid: args.pmcid,
    title: args.title,
    ...
    candidates: [],
    identifiers: { doi: normalizeDoi(args.doi), pmid: args.pmid || "", pmcid: args.pmcid || "" },
    error: String(error).slice(0, 160),
  }));
  const attempts = [];
  const candidates = rankCandidatesByHealth(
    discovery.candidates || [],
    loadSourceHealth()
  );
  for (const candidate of candidates) {
    process.stderr.write(`[oa] ${candidate.source} -> ${candidate.url}\n`);
    const startedAt = Date.now();
    const result = await downloadPdfUrl(
      args.proxy,
      candidate.url,
      args.out,
      args.title || "",
      { allowBrowser: !args.openAccess }
    ).catch((error) => ({
    ...
    recordSourceResult(candidate.source, success, latencyMs);
    if (success) {
      return {
        ...result,
        doi: discovery.identifiers?.doi || normalizeDoi(args.doi),
        pmid: discovery.identifiers?.pmid || args.pmid || "",
        pmcid: discovery.identifiers?.pmcid || candidate.pmcid || args.pmcid || "",
        source: candidate.source,
        via: candidate.url,
        attempts,
        source_latency_ms: latencyMs,
        transport_attempts: result.attempts || 1,
      };
    }
    attempts.push({
      source: candidate.source,
      url: candidate.url,
      error: result.err || result.status,
      latency_ms: latencyMs,
      transport_attempts: result.attempts || 1,
    });
  }

  const resolvedDoi = discovery.identifiers?.doi || normalizeDoi(args.doi);
  if (!args.openAccess && resolvedDoi) {
    process.stderr.write(`[publisher] OA sources exhausted; trying authenticated publisher route for ${resolvedDoi}\n`);
    const result = await downloadDoi(args.proxy, resolvedDoi, args.out, args.si, args.debug);
    return { ...result, title: args.title || "", source: result.source || "publisher", attempts };
  }
  return {
    title: args.title || "",
    doi: resolvedDoi,
    pmid: discovery.identifiers?.pmid || args.pmid || "",
    pmcid: discovery.identifiers?.pmcid || args.pmcid || "",
    status: STATUS.NO_AUTHORIZED_PDF_FOUND,
    source: "open-access-resolver",
    attempts,
    err: discovery.error || "no verified PDF from PMC, Europe PMC, OpenAlex, Semantic Scholar, DOAJ, or arXiv",
  };
}
...
async function main() {
  ...
  // Public OA mode stays browser-free. Institutional mode starts with OA but
  // keeps the authenticated browser fallback available.
  if (!args.openAccess) await healthCheck(args.proxy);

  const results = [];
  const t0 = Date.now();

  if (args.pdfUrl) {
    const r = await downloadPdfUrl(
      args.proxy,
      args.pdfUrl,
      args.out,
      args.title || "",
  ...
  if (args.title && !args.topic && (args.openAccess || args.doi || args.pmid || args.pmcid)) {
    const r = await downloadResolvedItem(args).catch((e) => ({
      title: args.title,
